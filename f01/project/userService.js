...
const userService = {
  createUser: (name, email) => {
    ...
    const nextId = data.users.length
      ? Math.max(...data.users.map((u) => u.id || 0)) + 1
      : 1;
  ...
  getUsers: () => {
    try {
      ...
      if (Array.isArray(parsed)) console.log(JSON.stringify(parsed, null, 2));
        ...
        console.log(JSON.stringify(parsed.users, null, 2));
      else console.log("[]");
    } catch (err) {
      console.log("ERROR: unable to read users file.");
      process.exit(1);
  ...
  deleteUser: (id) => {
    ...
    if (!Number.isNaN(numeric))
      index = data.users.findIndex((u) => u.id === numeric);
    if (index === -1) index = data.users.findIndex((u) => u.email === id);
    if (index === -1) {
      if (!Number.isNaN(numeric))
        console.log(`Error: User with id ${numeric} not found`);
